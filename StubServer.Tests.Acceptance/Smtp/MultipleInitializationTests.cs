...
{
    internal class MultipleInitializationTests : SmtpStubServerTests
    {
        [Test]
        public void Should_initialize_function()
        {
            // Arrange
            var smtpStubServer = new SmtpStubServer(IPAddress.Loopback, 5050, new Func<byte[]>[]
            {
                () => Encoding.ASCII.GetBytes("220 "),
                () => Encoding.ASCII.GetBytes("SMTP "),
                () => Encoding.ASCII.GetBytes("StubServer\r\n")
            });
        ...
        {
            // Arrange
            var smtpStubServer = new SmtpStubServer(IPAddress.Loopback, 5050, new Func<Task<byte[]>>[]
            {
                () => Task.FromResult(Encoding.ASCII.GetBytes("220 ")),
                () => Task.FromResult(Encoding.ASCII.GetBytes("SMTP ")),
                () => Task.FromResult(Encoding.ASCII.GetBytes("StubServer\r\n"))
            });
        ...
        {
            // Arrange
            var smtpStubServer = new SmtpStubServer(IPAddress.Loopback, 5050, new Func<CancellationToken,Task<byte[]>>[]
            {
                token => Task.FromResult(Encoding.ASCII.GetBytes("220 ")),
                token => Task.FromResult(Encoding.ASCII.GetBytes("SMTP ")),
                token => Task.FromResult(Encoding.ASCII.GetBytes("StubServer\r\n"))
            });
